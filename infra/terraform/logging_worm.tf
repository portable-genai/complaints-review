...
#         bucket whose retention is var.retention_days (~7 years) and whose `locked = true`
...
resource "google_logging_project_bucket_config" "worm_audit" {
  ...
  description    = "WORM audit bucket for B6 Complaints & Conduct File Review (locked, ~7y)."
  ...
  cmek_settings {
    kms_key_name = google_kms_crypto_key.complaints.id
  }
...
resource "google_logging_project_sink" "audit_to_worm" {
  ...
  description = "Routes the complaints-review-audit log to the locked WORM bucket."
