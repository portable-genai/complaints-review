...
def gcp_settings():
    ...
    # Force the managed stack regardless of the ambient COMPLAINTS_PROFILE.
    return Settings(
...
def container(gcp_settings):
    from complaints_review.config import Container
...
def test_knowledge_base_liveness(container):
    ...
    passages = container.knowledge_base.search(
        RetrievalQuery(text="complaint handling deadline policy", top_k=3)
    )
