...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    "CHROME_PATH": (
        "scripts/complaints_demo_playwright.py: which local Chrome binary Playwright drives. It "
    ...
    "DEMO_URL": (
        "scripts/complaints_demo_playwright.py: which already-running demo server the browser "
    ...
    "SLOWMO_MS": (
        "scripts/complaints_demo_playwright.py: how long Playwright pauses between steps so a "
        "human can follow the walkthrough. Pure presentation pacing; it reaches no service and "
        "grants nothing."
    ),
}
