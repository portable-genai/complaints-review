...
from complaints_review import config
from complaints_review.adapters.live.llm import LocalModelLLMAdapter
from complaints_review.config import LocalSettings, Settings, build_container, end_user_auth_kind
from complaints_review.domain.models import LlmMessage, LlmRequest, TokenUsage
from complaints_review.ports.identity import VERIFIED
...
def _request(**overrides: Any) -> LlmRequest:
    fields: dict[str, Any] = {
        "messages": (LlmMessage(role="user", content="Summarise the complaint."),),
        "system_instruction": "You review a customer complaint file.",
        "temperature": 0.0,
...
def test_a_fenced_invalid_first_answer_is_retried_and_the_valid_one_returned() -> None:
    transport = _FakeTransport(
        '```json\n{"summary": "wrong field"}\n```',
        '```json\n{"narrative": "The customer disputes a fee."}\n```',
    )
    response = _adapter(transport).generate(_request())

    assert json.loads(response.text) == {"narrative": "The customer disputes a fee."}
    ...
    assert "review a customer complaint file" in system["content"]
...
def test_classify_coerces_the_answer_onto_a_label() -> None:
    transport = _FakeTransport("The label is fees.")
    assert _adapter(transport).classify(
        "Customer disputes an overdraft fee", ["conduct", "fees"]
    ) == ("fees")
