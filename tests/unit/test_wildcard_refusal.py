...
``COMPLAINTS_CORS_ORIGINS`` decides who may call this service from a browser (with credentials,
because the middleware sets ``allow_credentials=True``) and ``COMPLAINTS_FRAME_ANCESTORS``
decides who may frame the review UI. A wildcard in either is the whole origin policy switched
off.
...
iframe presents ``null`` as its origin, so the accepted value hands the UI to any page that
...
from complaints_review.api.app import (
    _CORS_ORIGINS_ENV,
