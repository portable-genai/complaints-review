...
def _diverging_args() -> Namespace:
    ...
    return Namespace(
        package="acme_conduct_agent",
        cli="acme-conduct",
        env_prefix="ACME",
        resource="acme-conduct-review",
        dist="acme-conduct-dist",
    )
...
def test_dist_cli_and_resource_each_land_on_their_own_value() -> None:
    ...
    assert rewritten == (
        'name = "acme-conduct-dist"\n'
        'acme-conduct = "acme_conduct_agent.cli.main:app"\n'
        'resource_id = "acme-conduct-review"\n'
        "ACME_PROFILE\n"
    )
    assert count == 4
