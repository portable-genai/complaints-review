...
able to import the vertical-neutral kernel **without** dragging in the complaint and
conduct artifacts it is going to rewrite.

So the primary assertion is executed, not read: a fresh interpreter imports
``complaints_review.domain.kernel`` and reports whether ``complaints_review.domain.models``
ended up in ``sys.modules``. Against the previous shim that subprocess printed the vertical
module, so this test was RED before the split.
"""
...
from complaints_review.domain import kernel, models

SRC = Path(__file__).resolve().parents[2] / "src"
KERNEL_PATH = SRC / "complaints_review" / "domain" / "kernel.py"
...
# The complaint / conduct artifacts a fork rewrites. None may live in the kernel.
VERTICAL_NAMES = (
    "Categorization",
    "Channel",
    "ComplaintCategory",
...
def _import_probe(module: str) -> list[str]:
    ...
    program = (
        ...
        "if m.startswith('complaints_review'))))\n"
    )
...
def test_importing_the_kernel_does_not_import_the_vertical_models() -> None:
    ...
    imported = _import_probe("complaints_review.domain.kernel")
    assert "complaints_review.domain.kernel" in imported
    assert "complaints_review.domain.models" not in imported, (
...
def test_the_vertical_models_do_import_the_kernel() -> None:
    ...
    imported = _import_probe("complaints_review.domain.models")
    assert "complaints_review.domain.kernel" in imported
...
def test_kernel_source_has_no_intra_package_imports() -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            assert not (node.module or "").startswith("complaints_review"), node.module
        elif isinstance(node, ast.Import):
            for alias in node.names:
                assert not alias.name.startswith("complaints_review"), alias.name
