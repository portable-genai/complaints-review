...
The ``live`` profile's model port. Complaint summaries, categorisation and draft responses
are produced by a model running on the operator's own machine, reached through the ONE
client every laptop ``live`` lane uses, :class:`hex_service_kit.localmodel.LocalModelClient`.
Its endpoint, model and timeout are the fleet-wide ``LOCAL_MODEL_URL``, ``LOCAL_MODEL`` and
...
so the API answers a failed model call the same way under both profiles.
"""
