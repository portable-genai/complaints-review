...
_PERSONAS: tuple[Principal, ...] = (
    ...
    Principal(
        ...
        principals=("group:complaints-analyst", "group:conduct", "group:complaints-approver"),
        tenant="demo-bank",
        assurance="local-demo",
    ...
    Principal(
        ...
        principals=("group:complaints-analyst",),
        tenant="other-bank",
        assurance="local-demo",
...
class LocalPersonaProfileError(IdentityError):
    ...
    #: The persona arrives on a header the CALLER writes, so a caller chooses who it
    #: is. That is a picker, not authentication, and the exposure guard reads this
    #: declaration to keep the routes it serves off the LAN.
...
class LocalPersonaIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # actually chosen. Reading a missing COMPLAINTS_PROFILE as "local" would bind this
        # adapter and hand every caller the first seeded persona. It is an IdentityError
        # subclass so the API turns the refusal into a 401 rather than a 500.
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "COMPLAINTS_PROFILE is not set, so the local profile was inherited rather "
                "than chosen; seeded dev personas authenticate nobody and are refused. Set "
                "COMPLAINTS_PROFILE=local deliberately for a dev or demo run, or "
                "COMPLAINTS_PROFILE=gcp for a real deployment."
            )
