...
Screens inbound complaint text and outbound draft responses through **Model Armor** on
the regional host ``modelarmor.asia-southeast1.rep.googleapis.com`` via
``sanitizeUserPrompt`` / ``sanitizeModelResponse``: prompt-injection, jailbreak,
sensitive-data and malicious-URL detection. Because B6 handles customer complaint PII,
this screen is mandatory in both directions (rule R1).
