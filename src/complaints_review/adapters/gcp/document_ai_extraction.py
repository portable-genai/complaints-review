...
Primary production extraction backend for B6: **Document AI** on the Gemini Enterprise
Agent Platform, pinned to a **regional** processor in ``asia-southeast1`` (Singapore) so
that complaint-document processing stays in-country for residency. The adapter calls the
processor's ``process_document`` and maps the result onto a domain
:class:`DocumentExtract` (form-field key/values plus the full text and page count).
...
class DocumentAiExtractionAdapter:
    """Extract structured fields + text from a complaint document via Document AI."""
    ...
    def extract(self, document_id: str, content: bytes, mime_type: str) -> DocumentExtract:
        ...
        return self._to_extract(document_id, result.document)
