...
class IapIdentityAdapter:
    ...
    #: Verifies a server-side assertion (signature, issuer, expiry, audience), so a
    #: caller cannot name itself. This is the one declaration that stands the exposure
    #: guard down, and it is only defensible because ``resolve`` refuses an
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # "/projects/<NUM>/apps/<ID>". Configure via COMPLAINTS_IAP_AUDIENCE; required in
        # secure mode.
        #
        # Deliberate collapse: UNSET and SET-AND-EMPTY both land on "", and ``resolve``
        # refuses on "". Both states are closed in the SAME direction, because the fallback
        # a value would otherwise inherit is "verify with no expected audience", which
        # accepts assertions minted for a different resource. There is no safe default to
        # keep, so neither state gets one.
        self._audience = read_env_setting("COMPLAINTS_IAP_AUDIENCE").value
    ...
    def resolve(self, ctx: RequestContext) -> Principal:
        ...
        if not self._audience:
            raise IapAudienceUnconfiguredError(
                "COMPLAINTS_IAP_AUDIENCE is not configured, so no IAP assertion can be verified "
