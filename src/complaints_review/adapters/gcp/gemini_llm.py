...
(system instruction, temperature only when the request pins one, max-output-tokens, a
...
:class:`TokenUsage`. After a successful call it notes the model it called
(:func:`hex_service_kit.provenance.note_model`) for the console's model pill.
...
class GeminiLLMAdapter:
    ...
    def generate(self, request: LlmRequest) -> LlmResponse:
        ...
        # What answered, for the console's model pill. No online search tool is attached to
        # this call, so it never notes a search.
    ...
    def _build_config(self, request: LlmRequest, types: Any) -> Any:
        ...
        # Free sampling is an ABSENT temperature, never 1.0: some models reject the parameter.
    ...
    def _thinking_level(level: ThinkingLevel, types: Any) -> Any:
        ...
        finer-grained scale is collapsed onto them, with MEDIUM and above treated as
        HIGH so conduct reasoning runs at full depth.
        """
        mapping = {
