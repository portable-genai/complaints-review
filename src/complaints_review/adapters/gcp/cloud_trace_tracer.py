...
Backs the domain tracer with **Cloud Trace via OpenTelemetry**. Each ``tracer.span(name)``
opens an OTel span for a unit of review work; token usage is recorded as span attributes
for FinOps. Message-content capture is **OFF**: spans carry structure and token counts,
never the complaint text or draft response, so customer PII never reaches a trace.
...
class CloudTraceTracerAdapter:
    ...
    def _get_tracer(self) -> Any:
        if self._tracer is None:
            ...
            self._tracer = trace.get_tracer("complaints-review")
        return self._tracer
