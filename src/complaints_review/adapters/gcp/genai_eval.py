...
AI SDK: ``vertexai.Client(...).evals``). It scores a golden dataset on the B6 metrics
(categorisation accuracy, groundedness, citation accuracy, PII safety) and maps the
result onto a domain :class:`EvalReport`. ``gate`` is the promotion check.
...
class GenAiEvalAdapter:
    ...
    def gate(self, target: str) -> bool:
        """Promotion gate: True iff ``target`` clears every B6 threshold."""
