"""Sensitive Data Protection (DLP) redaction adapter (A1 Guardrail Gateway, rule R1).

Implements :class:`PIIRedactionPort` against **Sensitive Data Protection / DLP** of the
Gemini Enterprise Agent Platform. Every complaint narrative, document extract and draft
response is de-identified at the boundary : before it reaches a model, the knowledge base
or the WORM audit sink : so customer PII is minimised to the model (P-04 / R1). The call
is regional (``projects/{project}/locations/{region}``) to keep inspection inside
...
# jurisdictions, using their RE2-safe form. DLP custom info types are matched with RE2, which
# has NO lookaround, so a Python-only row (e.g. the JP My Number lookarounds) would make DLP
# reject the whole inspect config with INVALID_ARGUMENT and fail every call: re2_pattern_for
...
class DlpRedactionAdapter:
    ...
    def _custom_info_type_names(self) -> list[str]:
        ...
        names: list[str] = []
        for info_type, _pattern, _validator in national_patterns_for(
            self._settings.pii.jurisdictions
        ):
    ...
    def _custom_info_types(self) -> list[dict[str, Any]]:
        """DLP custom info types built from the pii-kit rows, using RE2-safe patterns.

        Rows that share an info type under two shapes (HK's parenthesised and bare HKID) are
        OR-ed into one RE2 alternation, so each info-type name appears once.
        """
        by_name: dict[str, list[str]] = {}
        for info_type, pattern, _validator in national_patterns_for(
            self._settings.pii.jurisdictions
        ):
