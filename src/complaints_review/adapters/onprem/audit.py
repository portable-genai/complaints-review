...
profile this port binds to the Cloud Logging locked-WORM-bucket adapter (or A5);
...
adapter, so the contract tests prove interface parity. ``record`` deliberately raises
rather than discarding the event: an unimplemented audit sink must never silently drop a
compliance record, so porting on-premise *must* supply a real immutable (WORM) audit
store. Filling this body in is the only change required.
"""
