...
profile this port binds to the Gen AI evaluation-service adapter (or A4); switching
...
adapter, so the contract tests prove interface parity. ``evaluate`` deliberately raises
rather than returning a vacuously-passing report: an unimplemented promotion gate must
never wave a model through unevaluated, so porting on-premise *must* supply a real eval
