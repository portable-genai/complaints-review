...
profile this port binds to Model Armor (or the A1 gateway); switching ``profile`` to
...
the contract tests prove interface parity. ``screen`` deliberately raises rather than
fail-opening: an unimplemented guardrail must never silently allow traffic, so porting
on-premise *must* provide a real screening backend. Filling this body in is the only
change required.
"""
