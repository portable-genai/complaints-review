...
When B6 runs inside the full Gemini Enterprise Agent Platform deployment, guardrail
...
:class:`GuardrailVerdict` (SPEC §6, A1 contract). Mandatory for B6 (rule R1).

The adapter follows the construction convention ``__init__(self, settings)`` and reads
its base URL from ``GUARDRAIL_GATEWAY_URL`` (sensible localhost default), so nothing
GCP-specific is required to construct or exercise it.
"""
...
class RemoteGuardrailError(ComplaintsError):
...
class RemoteGuardrailAdapter:
    ...
    def screen(self, text: str, direction: Direction) -> GuardrailVerdict:
        """Screen inbound complaint text or an outbound draft via the A1 gateway."""
