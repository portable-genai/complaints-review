...
(``COMPLAINTS_GUARDRAIL``, ``COMPLAINTS_PII_REDACTION``, ``COMPLAINTS_REVIEW_ROUTING``), the
...
* :class:`DisclosingRedaction` notes whether redaction changed the complaint the user
  submitted, which the console discloses rather than leaving the user to wonder why a
  summary reads "[PERSON_NAME]".
* :class:`RecordingReviewRouter` records whether a required hand-off reached the console,
  ...
  audited review must not fail on a console outage, but it is no longer invisible: the
...
from ..domain.models import ComplaintReview, Direction, GuardrailVerdict, RedactionResult
...
#: What each outcome means, in the words the operator needs. A review that requires a human
...
REVIEW_ROUTING_TEXT: dict[ReviewRouting, str] = {
    ...
    ReviewRouting.FAILED: (
        "Could not reach the review console; this review is not queued for a checker."
    ),
    ReviewRouting.OFF: (
        "Review routing is off in this deployment; this review is not queued for a checker."
    ),
...
class DisabledReviewRouter:
    ...
    def route(self, review: ComplaintReview, *, maker: str, tenant: str = "") -> None:
        return None
...
class RecordingReviewRouter:
    ...
    The service hands a review over only when it requires one, so a call that never reaches
    ...
    def route(self, review: ComplaintReview, *, maker: str, tenant: str = "") -> None:
        if not getattr(self._inner, "enabled", True):
            ...
            return
        try:
            self._inner.route(review, maker=maker, tenant=tenant)
