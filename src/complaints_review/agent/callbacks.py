...
every response that leaves it is, once more,
  ...
  1. **redacted** (Sensitive Data Protection / DLP, via :class:`PIIRedactionPort`) so
     customer PII never reaches the model or any log/span, and
...
The callbacks are built from a :class:`~complaints_review.config.Container`, so the active
...
agent-build time. The Cloud Trace adapter additionally disables content capture.

ADK imports are done lazily inside the factory / callbacks so this module imports without
ADK installed (SPEC §4). Callback signatures follow ADK 2.x.
"""
...
_LAST_PROMPT_KEY = "_cr_last_redacted_prompt"
_LAST_RESPONSE_KEY = "_cr_last_redacted_response"
_BLOCKED_KEY = "_cr_turn_blocked"
...
def _redact_then_screen(
    ...
    Order matters: redact first so PII never reaches the guardrail service either.
    """
...
def build_callbacks(
    ...
    ``google.adk`` is imported lazily here so the module is import-safe without ADK
    installed (SPEC §4).
    """
...
def _actor(callback_context: Any) -> str:
    ...
    return str(user_id) if user_id else "complaints-review-agent"
