...
class LlmRequest:
    ...
    #: Sampling is decided per call. ``None`` means the adapter SENDS NO temperature (free),
    #: which is not ``1.0``: some models reject the parameter outright. Pin ``0.0`` where the
    #: output is extracted, classified, scored or compared; the grounded builder
    #: (``_grounded.build_llm_request``) pins by default, so freeing a call is explicit.
