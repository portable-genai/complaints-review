...
| P-03 | Data residency / in-country | Every managed resource takes its location from `region`, which is chosen at deploy time and validated in Terraform against the `allowed_regions` residency allowlist (default `asia-southeast1`; extending that list is the review point); a VPC-SC perimeter (`vpc_sc.tf`) blocks exfiltration. |
