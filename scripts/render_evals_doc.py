...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_complaints.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden complaints.", ""]
    return lines


def _exercised_block() -> list[str]:
    cases = load_jsonl(GOLDEN)
    planted = [row for row in cases if row.get("pii_jurisdiction")]
    must_cite = sum(len(row.get("must_cite_source_ids", [])) for row in cases)
    markets = sorted({str(row["pii_jurisdiction"]) for row in planted})
    return [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden complaints** in `eval/datasets/golden_complaints.jsonl`, each",
        f"  with the category a reviewer assigned and {must_cite} must-cite source ids between",
        "  them. The expectation is the dataset's, never a re-read of the pipeline's verdict.",
        f"- **{len(planted)} of them plant a raw identifier**, across {len(markets)} markets",
        f"  ({', '.join(markets)}), so a broken pattern row for one jurisdiction cannot hide",
