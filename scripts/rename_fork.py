...
Rewrites the package name, the CLI entry point, the ``COMPLAINTS_`` env-var prefix, and the
...
wiring, escalation policy numbers, region) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "complaints_review"
_OLD_CLI = "complaints-review"
_OLD_ENV_PREFIX = "COMPLAINTS_"
_OLD_RESOURCE = "complaints-review"
_OLD_DIST = "complaints-review"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``COMPLAINTS_`` when it prefixes an UPPER token
        # (COMPLAINTS_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-conduct-review"
    )
    ...
    print(
        "Then finish the human steps in docs/ADOPTING.md (fixtures, IdP, escalation policy, region)."
    )
    return 0
