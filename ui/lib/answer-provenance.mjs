...
//
// This console has no same-origin route handler: it calls its service directly on the API base
// `lib/api` resolves from NEXT_PUBLIC_API_BASE (the service's own origin standalone, a
// root-relative mount under the portal). So "the console's own API" is that base, not a fixed
// `/api/agent` path, and a cross-origin standalone run can read the two headers only because the
// service's CORS policy exposes them (`expose_headers` in `api/app.py`).
...
/**
 * True when `input` addresses this console's API: the same origin as `base` and a path under it.
 *
 * @param {unknown} input what was handed to `fetch`
 * @param {string} here the page's own URL, which a relative `input` or `base` resolves against
 * @param {string} base the console's API base (absolute, or root-relative)
 */
export function isConsoleApi(input, here, base) {
  let raw;
 ...
 * @param {string} base the console's API base, as `lib/api` resolves it
 * @param {(answer: { model: string, search: boolean }) => void} listener
 * @returns {() => void} unsubscribe
 */
export function watchAnswers(host, base, listener) {
  ...
  if (!installed) {
    ...
    const wrapped = async (input, init) => {
      ...
      if (isConsoleApi(input, host.location.href, base)) {
