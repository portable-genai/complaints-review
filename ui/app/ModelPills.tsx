 ...
 * (`install_answer_provenance` in `api/app.py`), which also exposes them to a cross-origin
 * console through its CORS policy.
 *
...
// This console has no `/api/agent` route handler, so a pill reading that path would fetch
// nothing and, by the rule below, render nothing: silently, on every page load. Sharing the base
// is also what makes health reachable, because `connect-src` and the service's CORS allowlist are
// built for that same value.
