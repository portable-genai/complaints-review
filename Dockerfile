# B6 Complaints & Conduct File Review : API service image.
#
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    COMPLAINTS_SETTINGS=/app/config/settings.yaml \
    PORT=8095
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD python -c "import urllib.request,os; urllib.request.urlopen('http://127.0.0.1:'+os.environ.get('PORT','8095')+'/healthz')" || exit 1

CMD exec uvicorn complaints_review.api.app:app --host 0.0.0.0 --port ${PORT}
